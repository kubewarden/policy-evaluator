    ...
    match cert_chain {
        ...
        Some(cert_chain) => {
            ...
            let result = end_entity_certificate
                ...
                .map_or_else(
                    |e| match e {
                        Error::InvalidSignatureForPublicKey => BoolWithReason::False(
                            "Certificate is not trusted by the provided cert chain".to_string(),
                        ),
                        _ => BoolWithReason::False(format!("Certificate not trusted: {}", e)),
                    },
                    |_| BoolWithReason::True,
                );
            Ok(result)
        }
    }
...
mod tests {
    ...
    fn certificate_is_trusted() {
        ...
        let result = verify_certificate(req);
        assert!(matches!(result, Ok(BoolWithReason::True)));
    }
    ...
    fn certificate_is_not_trusted() {
        ...
        let result = verify_certificate(req).expect("Failed to verify certificate");
        assert!(
            matches!(result, BoolWithReason::False(reason) if reason == "Certificate is not trusted by the provided cert chain")
        );
    }
    ...
    fn certificate_is_expired() {
        ...
        let result = verify_certificate(req).expect("Failed to verify certificate");
        assert!(
            matches!(result, BoolWithReason::False(reason) if reason == "Certificate is being used after its expiration date")
        );
    }
    ...
    fn certificate_is_used_before_notbefore_date() {
        ...
        let result = verify_certificate(req).expect("Failed to verify certificate");
        assert!(
            matches!(result, BoolWithReason::False(reason) if reason == "Certificate is being used before its validity date")
        );
    }
}
